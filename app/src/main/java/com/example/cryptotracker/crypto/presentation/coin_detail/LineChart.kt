...
fun LineChart(
    ...
    Canvas(
        ...
        val minLabelSpacingYPx = style.minYLableSpacing.toPx()
        val verticalPaddingPx = style.verticalPadding.toPx()
        val horizontalPaddingPx = style.horizantalPadding.toPx()
        val xAxisLabelSpacingPx = style.xAxisLableSpacing.toPx()
            ...
            if(showHelperLines) {
                drawLine(
                    ...
                    strokeWidth = if(selectedDataPointIndex == index) {
                        style.hyperLinesThicknessPx * 1.8f
                    } else style.hyperLinesThicknessPx
                )
            }

            if(selectedDataPointIndex == index) {
                val valueLabel = ValueLable(
                    value = visibleDataPoints[index].y,
                    unit = unit
                )
                val valueResult = measurer.measure(
                    text = valueLabel.foramtted(),
                    style = textStyle.copy(
                        color = style.selectecColor
                    ),
                    maxLines = 1
                )
        ...
        yLabelTextLayoutResults.forEachIndexed { index, result ->
            ...
            if(showHelperLines) {
                drawLine(
                    ...
                    strokeWidth = style.hyperLinesThicknessPx
                )
            }
        }
...
private fun LineChartPreview() {
    CryptoTrackerTheme {
        ...
        val style = ChartStyle(
            ...
            selectecColor = Color.Black,
            hyperLinesThicknessPx = Stroke.HairlineWidth,
